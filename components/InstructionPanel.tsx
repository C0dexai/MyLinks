...
interface InstructionPanelProps {
    ...
    onUpdateInstructions: (instructions: Instruction) => Promise<void>;
    openAiConfig: OpenAiConfig | undefined;
    onUpdateOpenAiConfig: (config: OpenAiConfig) => Promise<void>;
}
...
const InstructionPanel: React.FC<InstructionPanelProps> = ({ isOpen, onClose, instructions, onUpdateInstructions, openAiConfig, onUpdateOpenAiConfig }) => {
    ...
    }, [isOpen, instructions, openAiConfig, defaultSystemInstruction, defaultAiInstruction]);

    const handleSave = useCallback(async () => {
        setSaveStatus('saving');
        try {
            await onUpdateInstructions({
                id: 1, // There's only one set of instructions
                system: systemInstruction,
                ai: aiInstruction,
            });
            await onUpdateOpenAiConfig({
            ...
            setSaveStatus('idle'); // Or show an error message
        }
    ...
    return (
        ...
        >
            ...
            <div className="flex-grow overflow-y-auto pr-2">
                <div className="mb-4">
                    <label htmlFor="openai-api-key" className="block text-sm font-medium text-gray-400 mb-1">OpenAI API Key:</label>
            ...
            <div className="flex-shrink-0 pt-4 mt-auto border-t border-[rgba(255,255,255,.15)] flex items-center justify-end gap-4">
                {saveStatus === 'saved' && <span className="text-sm text-neon-green transition-opacity duration-300">Settings Saved!</span>}
                <button
                    onClick={handleSave}
